...
plt.rcParams['legend.fontsize'] = 8  # Smaller for 23 values
plt.rcParams['figure.titlesize'] = 16

# Import colormap for better color distinction with 23 epsilon values
import matplotlib.cm as cm
from matplotlib.colors import Normalize

def get_epsilon_colormap(epsilon_values):
    """Generate a perceptually uniform colormap for epsilon values"""
...
# Note: PRODUCTION_START and TOTAL_STEPS may vary between old (eps<=0.50) and new (eps>0.50) scripts
# Old script: 600000 start, 2600000 total (4000 ps production)
# New script: 600000 start, 1600000 total (2000 ps production)
PRODUCTION_START = 600000  # step where production begins
TOTAL_STEPS = 2600000  # total simulation steps (maximum)
...
class ThermodynamicAnalyzer:
    ...
    def plot_temperature_evolution(self):
        """Plot temperature evolution with improved aesthetics for 23 epsilon values"""
        color_map = get_epsilon_colormap(self.epsilon_values)
        
        # Create TWO separate figures instead of cramped subplots
        # Figure 1: Time series - large with legend outside
        ...
        ax1.axhline(TARGET_TEMP, color='red', linestyle='--', linewidth=3, label='Target (300 K)', zorder=10)
        ax1.set_xlabel('Time (ns)', fontsize=14, fontweight='bold')
        ax1.set_ylabel('Temperature (K)', fontsize=14, fontweight='bold')
        ax1.set_title('Temperature Evolution - All Epsilon Values', fontsize=16, fontweight='bold')
        ax1.legend(loc='upper left', bbox_to_anchor=(1.02, 1), ncol=2, fontsize=8, frameon=True, shadow=True)
        ax1.grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(PLOTS_DIR / "01a_temperature_timeseries.png", dpi=600, bbox_inches='tight')
        plt.close()
        print("  Saved: 01a_temperature_timeseries.png")
        
        # Figure 2: Mean values with colorful bars
        ...
        colors_list = [color_map[eps] for eps in valid_eps]
        
        ax2.bar(x_pos, means, yerr=stds, capsize=5, 
               color=colors_list, alpha=0.8, edgecolor='black', linewidth=1.5)
        ax2.axhline(TARGET_TEMP, color='red', linestyle='--', linewidth=3, label='Target (300 K)', zorder=10)
        
        ax2.set_xticks(x_pos)
        ax2.set_xticklabels([f'{eps:.2f}' for eps in valid_eps], rotation=45, ha='right', fontsize=9)
        ax2.set_xlabel('C-O Interaction Strength ε (kcal/mol)', fontsize=14, fontweight='bold')
        ax2.set_ylabel('Mean Temperature (K)', fontsize=14, fontweight='bold')
        ax2.set_title('Average Temperature vs Epsilon', fontsize=16, fontweight='bold')
        ax2.legend(fontsize=12)
        ax2.grid(True, alpha=0.3, axis='y')
        
        plt.tight_layout()
        plt.savefig(PLOTS_DIR / "01b_temperature_summary.png", dpi=600, bbox_inches='tight')
        plt.close()
        print("  Saved: 01b_temperature_summary.png")
    ...
    def plot_pressure_evolution(self):
        ...
        ax3.set_xticklabels([f'{eps:.2f}' for eps in valid_eps], fontsize=8)
    ...
    def plot_density_analysis(self):
        ...
        ax2.set_xticks(x_pos)
        ax2.set_xticklabels([f'{eps:.2f}' for eps in valid_eps], fontsize=8)
    ...
    def export_comprehensive_csv(self):
        """Export detailed CSV data for module 1"""
        import pandas as pd
        ...
        if hasattr(self, 'data') and self.data:
            ...
            if timeseries_data:
                ...
                csv_file = PLOTS_DIR / f"module01_timeseries_data.csv"
        ...
        if hasattr(self, 'stats_df'):
            summary_file = PLOTS_DIR / f"module01_summary_stats.csv"
...
def main():
    ...
    epsilon_values = epsilon_values_list  # Use the global list defined at top
