...
class HighPriorityAnalyzer:
    def __init__(self, base_dir):
        ...
        self.epsilon_values = epsilon_values_list  # Use the global list defined at top
        self.epsilon_dirs = {}
    ...
    def plot_c60_diffusion(self):
        ...
        for eps, data in self.results['c60_diffusion'].items():
            ax1.plot(data['time_ps']/1000, data['msd'], 
                    label=f'ε={eps:.2f}', 
                    linewidth=1.5,
                    color=color_map[eps],
                    alpha=0.7)
        ...
        ax1.set_ylabel('MSD (Å²)', fontsize=14, fontweight='bold')
        ax1.set_title('C60 Mean Square Displacement', fontsize=16, fontweight='bold')
    ...
    def plot_thermodynamic_properties(self):
        ...
        # Heat capacity
        ax1.plot(epsilons, C_v_values, 'o-', linewidth=2, markersize=10, color='crimson', markeredgecolor='black')
        ax1.set_xlabel('ε (kcal/mol)', fontsize=14, fontweight='bold')
        ax1.set_ylabel('C_v (kcal/(mol·K·atom))', fontsize=14, fontweight='bold')
        ax1.set_title('Specific Heat Capacity', fontsize=16, fontweight='bold')
        ax1.grid(True, alpha=0.3)
        
        # Compressibility
        ax2.plot(epsilons, kappa_values, 's-', linewidth=2, markersize=10, color='navy', markeredgecolor='black')
        ...
        ax2.set_ylabel('κ_T (GPa⁻¹)', fontsize=14, fontweight='bold')
        ax2.set_title('Isothermal Compressibility', fontsize=16, fontweight='bold')
        ax2.legend()
    ...
    def export_comprehensive_csv(self):
        """Export detailed CSV data for module 7"""
        import pandas as pd
        ...
        if hasattr(self, 'data') and self.data:
            ...
            if timeseries_data:
                ...
                csv_file = PLOTS_DIR / f"module07_timeseries_data.csv"
        ...
        if hasattr(self, 'stats_df'):
            summary_file = PLOTS_DIR / f"module07_summary_stats.csv"
            self.stats_df.to_csv(summary_file, index=False, float_format='%.6f')
            print(f"  Exported summary: {summary_file.name}")

    def export_comprehensive_csv(self):
        """Export detailed CSV data for Module 07"""
        import pandas as pd
        
        # 1. C60 Distances
        if 'c60_distances' in self.results:
            distance_data = []
            for eps, data in self.results['c60_distances'].items():
                df = data['data'].copy()
                df['Epsilon'] = eps
                distance_data.append(df)
            
            if distance_data:
                combined_dist = pd.concat(distance_data, ignore_index=True)
                csv_file = self.plots_dir / "module07_c60_distances.csv"
                combined_dist.to_csv(csv_file, index=False, float_format='%.6f')
                print(f"  ✓ Exported: {csv_file.name}")
        
        # 2. Diffusion Data
        if 'c60_diffusion' in self.results:
            diffusion_data = []
            for eps, data in self.results['c60_diffusion'].items():
                df = pd.DataFrame({
                ...
                diffusion_data.append(df)
            
            if diffusion_data:
                combined_diff = pd.concat(diffusion_data, ignore_index=True)
                csv_file = self.plots_dir / "module07_c60_msd_data.csv"
                combined_diff.to_csv(csv_file, index=False, float_format='%.6f')
                print(f"  ✓ Exported: {csv_file.name}")
