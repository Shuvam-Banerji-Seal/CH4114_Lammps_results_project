...
class StructuralAnalyzer:
    ...
    def plot_structural_results(self):
        ...
        if hasattr(self, 'results_hydration'):
            ...
            ax1.plot(epsilons, waters_in_shell, 'o-', linewidth=2, markersize=8, color='dodgerblue', markeredgecolor='black')
            ax1.set_xlabel('ε (kcal/mol)', fontsize=14, fontweight='bold')
            ax1.set_ylabel('Water molecules in first shell', fontsize=14, fontweight='bold')
            ax1.set_title('Hydration Shell Population', fontsize=16, fontweight='bold')
            ax1.grid(True, alpha=0.3)
            
            ax2.plot(epsilons, mean_distances, 's-', linewidth=2, markersize=8, color='crimson', markeredgecolor='black')
            ax2.set_xlabel('ε (kcal/mol)', fontsize=14, fontweight='bold')
            ax2.set_ylabel('Mean O-C60 distance (Å)', fontsize=14, fontweight='bold')
            ax2.set_title('Hydration Shell Extent', fontsize=16, fontweight='bold')
    ...
    def export_comprehensive_csv(self):
        """Export detailed CSV data for module 10"""
        import pandas as pd
        
        # 1. Structural Integrity
        if hasattr(self, 'results_integrity') and self.results_integrity:
            df = pd.DataFrame(self.results_integrity).T
            df.index.name = 'Epsilon'
            df.reset_index(inplace=True)
            csv_file = self.plots_dir / "module10_structural_integrity.csv"
            df.to_csv(csv_file, index=False, float_format='%.6f')
            print(f"  ✓ Exported: {csv_file.name}")
            
        # 2. Hydration Shell
        if hasattr(self, 'results_hydration') and self.results_hydration:
            df = pd.DataFrame(self.results_hydration).T
            df.index.name = 'Epsilon'
            df.reset_index(inplace=True)
            csv_file = self.plots_dir / "module10_hydration_shell.csv"
            df.to_csv(csv_file, index=False, float_format='%.6f')
            print(f"  ✓ Exported: {csv_file.name}")
            
        # 3. Radial Distributions (Summary)
        if hasattr(self, 'results_radial') and self.results_radial:
            # We already export individual files in analyze_radial_distribution
            # But let's create a combined one if useful, or just skip
            pass
