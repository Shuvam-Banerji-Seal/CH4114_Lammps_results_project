...
class EquilibrationConvergenceAnalyzer:
    ...
    def analyze_equilibration_stages(self):
        """Analyze convergence of all 4 stages for ALL epsilons"""
        print("\n" + "="*80)
        ...
        for eps in self.epsilon_values:
            ...
            # Create comprehensive plots for this epsilon
        ...
        # After looping all epsilons, plot cross-epsilon summary
        self.plot_cross_epsilon_convergence()
    ...
    def _save_convergence_metrics(self, stage_data, eps):
        """Save convergence metrics to CSV and accumulate for summary"""
        metrics = []
        
        # We only care about the final stage (NPT) for the summary, 
        # but let's save all stages to the per-epsilon CSV
        npt_metrics = {}
        ...
        for stage_name, df in stage_data.items():
            ...
            if 'Volume' in df.columns:
                volume = df['Volume'].values
                density = 5541.0 / volume * 1.66054
                metric['Rho_final'] = density[-1]
                metric['Rho_avg_last100'] = density[-100:].mean() if len(density) > 100 else density.mean()
            
            metrics.append(metric)
            
            if stage_name == 'NPT':
                npt_metrics = metric.copy()
                npt_metrics['Epsilon'] = eps
        
        df_metrics = pd.DataFrame(metrics)
        csv_file = self.data_dir / f'convergence_metrics_eps{eps:.2f}.csv'
        df_metrics.to_csv(csv_file, index=False)
        
        # Accumulate NPT metrics for cross-epsilon summary
        if npt_metrics:
            self.all_metrics.append(npt_metrics)

    def plot_cross_epsilon_convergence(self):
        """Plot final equilibrated properties vs epsilon"""
        if not self.all_metrics:
            return
            
        print("\nGenerating cross-epsilon summary plots...")
        df = pd.DataFrame(self.all_metrics).sort_values('Epsilon')
        
        fig, axes = plt.subplots(2, 2, figsize=(14, 10))
        
        # Temperature
        ax = axes[0, 0]
        if 'T_avg_last100' in df.columns:
            ax.errorbar(df['Epsilon'], df['T_avg_last100'], yerr=df.get('T_std_last100', 0), 
                       fmt='o-', color='tab:blue', capsize=5)
            ax.axhline(300, color='red', linestyle='--', alpha=0.5, label='Target')
            ax.set_ylabel('Temperature (K)')
        ...
        # Pressure
        ax = axes[0, 1]
        if 'P_avg_last100' in df.columns:
            ax.errorbar(df['Epsilon'], df['P_avg_last100'], yerr=df.get('P_std_last100', 0), 
                       fmt='o-', color='tab:orange', capsize=5)
            ax.axhline(1, color='red', linestyle='--', alpha=0.5, label='Target')
            ax.set_ylabel('Pressure (atm)')
        ...
        # Density
        ax = axes[1, 0]
        if 'Rho_avg_last100' in df.columns:
            ax.plot(df['Epsilon'], df['Rho_avg_last100'], 'o-', color='tab:green')
        ...
        # Energy
        ax = axes[1, 1]
        if 'E_avg_last100' in df.columns:
            ax.plot(df['Epsilon'], df['E_avg_last100'], 'o-', color='tab:purple')
            ax.set_ylabel('Total Energy (kcal/mol)')
            ax.set_title('Final Total Energy')
            ax.grid(True, alpha=0.3)
            
        for ax in axes.flatten():
            ax.set_xlabel('ε (kcal/mol)')
            
        plt.suptitle('Equilibration Convergence Summary', fontsize=16, fontweight='bold')
        plt.tight_layout()
        plt.savefig(self.plots_dir / '37_equilibration_summary_all_eps.png', dpi=300)
        print("  ✓ Saved: 37_equilibration_summary_all_eps.png")


    def export_comprehensive_csv(self):
        """Export detailed CSV data for module 12"""
        import pandas as pd
        
        # Export summary metrics across all epsilons
        if hasattr(self, 'all_metrics') and self.all_metrics:
            df = pd.DataFrame(self.all_metrics).sort_values('Epsilon')
            csv_file = self.plots_dir / "module12_equilibration_summary.csv"
