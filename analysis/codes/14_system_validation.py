...
class SystemValidator:
    ...
    def create_validation_table(self):
        ...
        self.validation_results = all_data
    ...
    def create_validation_summary(self):
        ...
        for status, item in checklist:
            ...
            ax.text(0.15, y_pos, item, transform=ax.transAxes, fontsize=8)
            y_pos -= 0.11
        ...
        for param, value in water_info:
            ...
            ax.text(0.5, y_pos, value, transform=ax.transAxes, fontsize=8)
            y_pos -= 0.11
...
- eps=0.25: Maximum attraction
    ...
    def export_comprehensive_csv(self):
        """Export detailed CSV data for module 14"""
        import pandas as pd
        
        if hasattr(self, 'validation_results') and self.validation_results:
            # Export composition validation
            if 'composition' in self.validation_results:
                df = pd.DataFrame(self.validation_results['composition'])
                csv_file = self.plots_dir / "module14_system_composition.csv"
                df.to_csv(csv_file, index=False)
                print(f"  ✓ Exported CSV: {csv_file.name}")
            
            # Export force field validation
            if 'force_field' in self.validation_results:
                df = pd.DataFrame(self.validation_results['force_field'])
                csv_file = self.plots_dir / "module14_force_field_params.csv"
                df.to_csv(csv_file, index=False)
                print(f"  ✓ Exported CSV: {csv_file.name}")
            
            # Export epsilon validation
            if 'epsilon' in self.validation_results:
                df = pd.DataFrame(self.validation_results['epsilon'])
                csv_file = self.plots_dir / "module14_epsilon_validation.csv"
                df.to_csv(csv_file, index=False)
                print(f"  ✓ Exported CSV: {csv_file.name}")
...
def main():
    ...
    try:
        ...
        # Export data
        validator.export_comprehensive_csv()
