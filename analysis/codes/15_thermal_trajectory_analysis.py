...
class ThermalTrajectoryAnalyzer:
    ...
    def analyze_thermal_convergence(self):
        """Analyze thermal convergence across all stages for ALL epsilons"""
        print("\n" + "="*80)
        ...
        for eps in self.epsilon_values:
            ...
            # Create multi-panel thermal convergence plot for this epsilon
            ...
            # Save data
            self.save_thermal_analysis_data(stage_data, eps)
            
        # Plot summary across epsilons
        self.plot_thermal_summary()
    ...
    def save_thermal_analysis_data(self, stage_data, eps):
        """Save thermal analysis data to CSV and accumulate for summary"""
        analysis_metrics = []
        
        # We'll use NPT stage for the summary
        npt_metrics = {}
        ...
        if npt_metrics:
            self.all_thermal_metrics.append(npt_metrics)

    def plot_thermal_summary(self):
        """Plot thermal metrics vs epsilon"""
        if not self.all_thermal_metrics:
            return
            
        print("\nGenerating thermal summary plots...")
        df = pd.DataFrame(self.all_thermal_metrics).sort_values('Epsilon')
        
        fig, axes = plt.subplots(1, 3, figsize=(18, 5))
        
        # Temperature Stability
        ax = axes[0]
        if 'T_std_last100' in df.columns:
            ax.plot(df['Epsilon'], df['T_std_last100'], 'o-', color='tab:red', linewidth=2)
            ax.set_ylabel('Temperature Std Dev (K)')
            ax.set_title('Temperature Stability')
            ax.grid(True, alpha=0.3)
            
        # Energy Drift
        ax = axes[1]
        if 'E_drift_pct' in df.columns:
            ax.bar(df['Epsilon'], df['E_drift_pct'], width=0.04, color='tab:purple', alpha=0.7)
        ...
        # Final Temperature
        ax = axes[2]
        if 'T_mean_last100' in df.columns:
            ax.errorbar(df['Epsilon'], df['T_mean_last100'], yerr=df.get('T_std_last100', 0),
                       fmt='o-', color='tab:blue')
            ax.axhline(300, color='red', linestyle='--', alpha=0.5, label='Target')
            ax.set_ylabel('Final Temperature (K)')
            ax.set_title('Equilibrated Temperature')
            ax.grid(True, alpha=0.3)
            
        for ax in axes:
            ax.set_xlabel('ε (kcal/mol)')
            
        plt.suptitle('Thermal Stability Summary Across Epsilon Values', fontsize=14, fontweight='bold')
        plt.tight_layout()
        plt.savefig(self.plots_dir / '45_thermal_summary_all_eps.png', dpi=300)
        print("  ✓ Saved: 45_thermal_summary_all_eps.png")


    def export_comprehensive_csv(self):
        """Export detailed CSV data for module 15"""
        import pandas as pd
        
        # Export summary metrics across all epsilons
        if hasattr(self, 'all_thermal_metrics') and self.all_thermal_metrics:
            df = pd.DataFrame(self.all_thermal_metrics).sort_values('Epsilon')
            csv_file = self.plots_dir / "module15_thermal_summary.csv"
...
def main():
    ...
    try:
        # nvt_data = analyzer.analyze_nvt_thermalization() # Skip single eps NVT
        analyzer.analyze_thermal_convergence()
