...
class CUDATrajectoryAnalyzer:
    ...
    def export_comprehensive_csv(self):
        """Export detailed CSV data for module 16"""
        import pandas as pd
        ...
        if hasattr(self, 'results') and self.results:
            ...
            for eps in sorted(self.results.keys()):
                summary_data.append({
                    'epsilon': eps,
                    'Residence Time (ns)': self.results[eps]['mean_residence_ns'],
                    'Entropy': self.results[eps]['entropy']
                })
            
            df_sum = pd.DataFrame(summary_data)
            csv_file = self.plots_dir / "module16_comprehensive_metrics.csv"
            df_sum.to_csv(csv_file, index=False, float_format='%.6f')
